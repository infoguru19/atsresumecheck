...
export default function Results({ result, fileName, onReset, onBuild }) {
      ...
      <div style={{ textAlign:'center', paddingBottom:'3rem' }}>
        <div style={{ display:'flex', gap:'0.75rem', justifyContent:'center', flexWrap:'wrap' }}>
          <button onClick={onReset} style={{
            ...
            padding:'0.875rem 2rem', color:'#fff', fontSize:'0.9rem', fontWeight:700,
            boxShadow:'0 4px 14px rgba(47,200,154,0.4)', cursor:'pointer',
          ...
          {onBuild && (
            <button onClick={onBuild} style={{
              background:'#fff', border:'2px solid var(--teal)', borderRadius:'var(--radius-sm)',
              padding:'0.875rem 2rem', color:'var(--teal)', fontSize:'0.9rem', fontWeight:700,
              cursor:'pointer', transition:'all 0.2s',
            }}
              onMouseOver={e=>{ e.currentTarget.style.background='var(--teal-light)' }}
              onMouseOut={e=>{ e.currentTarget.style.background='#fff' }}
            >✏️ Build 100% ATS Resume</button>
          )}
        </div>
